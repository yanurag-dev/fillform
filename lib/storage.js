...
const KEYS = {
  ...
  SESSION_IMAGES: 'session_uploaded_images',
};
...
export async function clearSessionData() {
  await chrome.storage.session.remove([KEYS.SESSION_DATA, KEYS.SESSION_IMAGES]);
}

// --- Session Images (volatile, cleared on browser close) ---

export async function setSessionImages(images) {
  await chrome.storage.session.set({ [KEYS.SESSION_IMAGES]: images });
}

export async function getSessionImages() {
  const result = await chrome.storage.session.get(KEYS.SESSION_IMAGES);
  return result[KEYS.SESSION_IMAGES] ?? null;
}
